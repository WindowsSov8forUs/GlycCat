...
当前代码仍处于迁移修复阶段。下文的“实现”表示代码中存在对应链路，不代表真实 QQ 账号已获权限或已经完成生产联调。未完成事项见文末，不能把构建通过当成功能验收完成。
...
示例仅说明结构，实际应透传完整 referrer。`<qq:passive id="..." seq="..."/>` 仍由 SDK 支持；不要写成 `<passive>`。展示引用使用 `<quote id="..."/>`，它不是被动回复来源。旧 `<quote><message id="..."/></quote>` 写法仍兼容：未填写 quote.id 时，应用只取直接子元素中首个非空 message.id 补齐；显式 quote.id 优先，引用内容和其他属性保持原样。这一兼容不生成 msg_id、event_id 或回复序号，新调用仍建议使用标准 quote.id。
...
`message.create` 会在发送前处理 data URI 和本服务所属账号的 internal 媒体。JPEG、PNG、GIF、AMR、SILK 及已识别的 MP4 保留原始字节；其他可解码图片转换为 JPEG/PNG，其他音频转换为 SILK，非 MP4 视频转换为 H.264/AAC 的 MP4。转换后同步更新媒体类型与文件名后缀，其他消息属性和回复上下文不变。

HTTP/HTTPS 地址仍原样交给 SDK，不在应用侧下载；引用内容和不由 SDK 发送的扩展子元素不触发媒体处理。频道音视频、频道私信本地图片等尚不支持的场景仍由 SDK 返回原状态。所有必要转换完成后才调用发送器，预处理失败不会先发出前面的文字；进入 SDK 后的分段部分成功语义保持不变。

单条消息本地媒体最多 64 个，读取的媒体输入总量及处理后的消息内容均不超过 32 MiB，包括内联 base64 的膨胀；最多并行预处理两条消息，每条共享 60 秒预算。这些是应用资源保护，不是 QQ 平台配额。MP4 容器识别不验证所有音视频编码，原格式能否被平台接受仍需实际确认。

视频和音频转码需要 PATH 中的 ffmpeg，最长处理时间为 60 秒，并有输出上限；不忽略当前目录可执行文件的安全检查。内嵌 SILK 编码器的 macOS 与 Windows 文件目前只有 amd64 可用，arm64 不能假定能原生执行。Linux 编码器仍有系统动态库依赖。辅助函数的真实输出和各平台执行效果尚待验证，不据此扩大支持承诺。
...
GlycCat 已为配置迁移、消息库与分页、媒体处理、运行流程等增加本地测试；运行 `go test ./...` 可检查这些逻辑。测试只覆盖模拟输入和本机可用的转码器，SDK 自带测试与真实 QQ 账号的业务联调仍是不同范围。发行配置按 go.mod 选择工具链，固定 GoReleaser 版本，不在发布时整理依赖；CI 发布任务是否通过以实际运行结果为准。

以下事项仍未完成，不应作为已验收功能：

- 发送前媒体预处理和旧嵌套引用兼容已有本地测试，MP4 与 SILK 转码也在有 ffmpeg 的测试环境运行；其他格式、平台及真实 QQ 发送效果仍需验证。
- 固定 SDK 的 WebHook 注册失败状态回滚、频道私聊创建返回的 ID/类型一致性，以及 SDK 自建 HTTP 监听的读头/空闲保护仍待独立修复；公网部署需在代理层提供相应限制。
- 新消息库、分页和迁移已有本地功能测试，真实旧库数据与 QQ 权限、频控、媒体效果及跨平台运行仍需独立验证。
