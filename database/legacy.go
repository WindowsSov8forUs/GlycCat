...
func MigrateMessages(ctx context.Context, sourcePath, targetPath, appID, selfID string) (result MigrationResult, resultErr error) {
	...
	sourceKey, targetKey := strings.ToLower(sourcePath), strings.ToLower(targetPath)
	if sourceKey == targetKey || strings.HasPrefix(targetKey, sourceKey+string(os.PathSeparator)) || strings.HasPrefix(sourceKey, targetKey+string(os.PathSeparator)) {
		return result, fmt.Errorf("新库与原库不能位于同一目录或相互包含")
	}
